...
export class ConnectionManager extends EventEmitter {
    ...
    updateConfigForTransportFallback() {
        if (!this.state.currentTransport)
            return;
    ...
    scheduleReconnect() {
        const backoffDelay = Math.min(1000 * Math.pow(2, this.state.reconnectAttempts), 30000 // Max 30 second delay
        );
        this.state.reconnectAttempts++;
        getLogger().sip.debug(`ConnectionManager: Scheduling reconnect attempt ${this.state.reconnectAttempts}/${this.state.maxReconnectAttempts} in ${backoffDelay}ms`);
    ...
    handleProviderSpecificError(error) {
    ...
    handleCiscoError(error) {
        ...
        if (error.message?.includes('transport') || error.message?.includes('TCP')) {
            ...
            this.config.preferredTransports = ['tcp', 'udp'];
            return true;
        }
    ...
    handleFritzBoxError(error) {
        ...
        if (error.message?.includes('transport') && this.config.preferredTransports?.includes('tcp')) {
            ...
            this.config.preferredTransports = ['udp'];
            return true;
        }
        return false;
    }
